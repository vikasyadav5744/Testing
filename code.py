...
with tab3:
    ...
    if newdata is not None:
        ...
        if spot2>0:
            ...
            with col1:
                st.write(f"""<div style="background-color: #7dc9aa; font-size:20px; padding: 5px; border-radius: 5px; text-align: center; margin:3px;"> {OICE_state.ce_status.iloc[0]}</div>""", unsafe_allow_html=True)
            with col2:
                st.write(f"""<div style="background-color: #7dc9aa; font-size:20px; padding: 5px; border-radius: 5px;text-align: center; margin:3px;">{OICEVOL_state.volce_status.iloc[0]}</div>""", unsafe_allow_html=True)
            with col3:
                st.write(f"""<div style="background-color: #7dc9aa; font-size:20px; padding: 5px; border-radius: 5px;text-align: center; margin:3px;">{OIPE_state.pe_status.iloc[0]} </div>""", unsafe_allow_html=True)
            with col4:
                st.write(f"""<div style="background-color:#7dc9aa; font-size:20px; padding:5px; border-radius: 5px;text-align: center; margin:3px;">VOLUME :- {OIPEVOL_state.volpe_status.iloc[0]}</div>""", unsafe_allow_html=True)
