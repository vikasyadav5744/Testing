...
with tab3:
    ...
    if newdata201 is not None:
        ...
        if spot2>0:
            ...
            with col1:
                st.write(f"""<div style="background-color: #916596; font-size:20px; padding: 5px; border-radius: 5px; text-align: center; margin:3px;"> {OICE_state.ce_status.iloc[0]}<p style='color:blue'>({len(OICE_state.ce_status)})</p></div>""", unsafe_allow_html=True)
            with col2:
