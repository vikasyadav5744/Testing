...
with tab3:
    ...
    if newdata201 is not None:
        ...
        if spot2>0:
            ...
            with col2:
                st.write(f"""<div style="background-color: #916596; font-size:20px; padding: 5px; border-radius: 5px;text-align: center; margin:3px;">{OICEVOL_state.volce_status.iloc[0]}<p style='color:blue'>({len(OICEVOL_state.volce_status)})</p></div>""", unsafe_allow_html=True)
            with col3:
                st.write(f"""<div style="background-color: #6d8a51; font-size:20px; padding: 5px; border-radius: 5px;text-align: center; margin:3px;">{OIPE_state.pe_status.iloc[0]}<p style='color:blue'>({len(OIPE_state.pe_status)})</p></div>""", unsafe_allow_html=True)
            with col4:
                st.write(f"""<div style="background-color:#6d8a51; font-size:20px; padding:5px; border-radius: 5px;text-align: center; margin:3px;"> {OIPEVOL_state.volpe_status.iloc[0]}<p style='color:blue'>({len(OIPEVOL_state.volpe_status)})</p></div>""", unsafe_allow_html=True)
